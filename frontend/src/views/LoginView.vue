<template>
  <div class="auth-page">
    <div class="auth-container">
      ...
      <div class="auth-card">
        <!-- 登录表单 -->
        <div v-if="!isRegister" class="auth-form">
          ...
          <p class="auth-switch">
            还没有账号？
            <span class="auth-link" @click="isRegister = true">立即注册</span>
          </p>
        ...
        <div v-else class="auth-form">
          ...
          <p class="auth-switch">
            已有账号？
            <span class="auth-link" @click="isRegister = false">去登录</span>
          </p>
          <p v-if="errorMsg" class="auth-error">{{ errorMsg }}</p>
        </div>
      </div>
...
const form = ref({ phone: '', password: '', code: '', fullName: '' })
...
async function handleLogin() {
  ...
  try {
    ...
    errorMsg.value = e.response?.data?.detail || '登录失败，请检查手机号和密码'
  } finally {
...
async function handleRegister() {
  errorMsg.value = ''
  if (!form.value.phone || !form.value.password || !form.value.code) {
