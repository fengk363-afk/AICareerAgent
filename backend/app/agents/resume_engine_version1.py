...
class ResumeEngine:
    ...
    async def parse_and_create(self, user_id, file_bytes: bytes, filename: str) -> ResumeProfileResponse:
        print("========== ENTER parse_and_create ==========")
        """上传 PDF → 解析 → 生成画像"""
        parsed_text = await self._extract_text(file_bytes)
        logger.info(f"PDF 解析完成，{len(parsed_text)} 字符")

        # DEBUG: 打印 PDF 提取文本前3000字符
        print(f"\n{'='*80}")
        print(f"[DEBUG] parsed_text (前3000字符, 共{len(parsed_text)}字符):")
        print(f"{'='*80}")
        print(parsed_text[:3000])
        print(f"{'='*80}\n")

        # 解析
        extracted = await self._extract_profile(parsed_text)

        # DEBUG: 打印 experience 和 project_experience
        print(f"\n{'='*80}")
        print(f"[DEBUG] extracted 结果:")
        print(f"  experience 数量: {len(extracted.get('experience', []))}")
        for i, exp in enumerate(extracted.get('experience', [])):
            print(f"    [{i}] {exp}")
        print(f"  project_experience 数量: {len(extracted.get('project_experience', []))}")
