...
from app.schemas.models import InterviewSessionResponse, InterviewQuestion, UserAnswer, InterviewFeedback, InterviewStatus
...
class InterviewEngine:
    ...
    async def create_session(self, user_id, job_id: str) -> Optional[InterviewSessionResponse]:
        """创建面试会话"""
    ...
    def _generate_questions(self, job: Job) -> List[InterviewQuestion]:
        """根据岗位类型生成面试问题（HR面+技术面+行为面）"""
        job_title = job.title
        for key in self.QUESTION_BANK:
            if key in job_title:
                bank = self.QUESTION_BANK[key]
                questions = []
                # 每种类型选1-2题
                for cat in ["technical", "behavioral", "situational"]:
                    if cat in bank:
                        questions.append(InterviewQuestion(
                            question=bank[cat][0]["question"],
                            category=cat,
                            difficulty=bank[cat][0]["difficulty"],
                        ))
                return questions
    ...
    async def _generate_feedback(
        ...
        for i, ans in enumerate(answers):
            ...
            feedback_list.append(InterviewFeedback(
                question_index=i, score=score, strengths=strengths,
                improvements=improvements, suggested_answer=suggested,
            ))
